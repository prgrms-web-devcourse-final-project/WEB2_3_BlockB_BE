 ...
 * <p>
 * 이 클래스는 채팅방의 고유 식별자(roomId)를 키(key)로, 해당 채팅방 정보를 담은 {@link DebateRoom} 객체를 값(value)으로 저장합니다.
...
public class DebateRoomStore {
	...
	private final Map<String, DebateRoom> debateRoomCache = new ConcurrentHashMap<>();

	/**
	 * 주어진 채팅방 정보를 캐시에 저장합니다.
	 *
	 * @param debateRoom 저장할 {@link DebateRoom} 객체
	 */
	public void put(DebateRoom debateRoom) {
		debateRoomCache.put(debateRoom.getRoomId(), debateRoom);
	}
	 ...
	 * @return 해당 roomId에 해당하는 {@link DebateRoom} 객체, 존재하지 않으면 null
	 */
	 ...
	 */
	public Map<String, DebateRoom> getAll() {
